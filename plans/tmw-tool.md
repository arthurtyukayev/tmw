...
allcore   crud-organizations   attached
allcore   auth-refactor        detached
mini-tv   player-rewrite       no session
```
...
Examples:
  ...
  tmw init allcore \
    --type=fullstack \
    --backend=~/Projects/allcore/allcore-backend \
    --frontend=~/Projects/allcore/allcore-frontend

  # Single repo project
  tmw init mini-tv \
    --type=single \
    --backend=~/Projects/Personal/mini-tv
```
...
Examples:
  tmw new allcore crud-organizations
  tmw new mini-tv player-rewrite

What happens (allcore example):
  git worktree add -b at/crud-organizations \
    .../allcore-backend-crud-organizations main
  git worktree add -b at/crud-organizations \
    .../allcore-frontend-crud-organizations main
  tmuxp load allcore.worktree  # with env vars set
```
...
Examples:
  tmw kill allcore crud-organizations
  tmw kill mini-tv player-rewrite

What happens (allcore example):
  tmux kill-session -t allcore-crud-organizations
  git worktree remove .../allcore-backend-crud-orgs
  git worktree remove .../allcore-frontend-crud-orgs
...
Examples:
  tmw attach allcore crud-organizations
  tmw attach mini-tv player-rewrite
```
...
~/.tmuxp/
  allcore.yaml                # Static (exists)
  allcore.worktree.yaml       # Parameterized (exists)
  mini-tv.yaml                # Static (exists)
  mini-tv.worktree.yaml       # Generated by init
  plans/
...
9. Update existing `allcore.worktree.yaml` to match
